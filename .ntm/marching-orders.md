...
1. **codex boot hooks-trust dialog** ("Review hooks / Trust all and continue") - settled by config on 2026-09-02: the ntm `[agents] codex` launch template (chezmoi-managed `~/.config/ntm/config.toml`) now runs `codex-account <account> --dangerously-bypass-hook-trust ...`, the only suppression that holds. The trust entry recorded where codex reads it (`[projects."/home/gabriel/repositories/agent-usage-book"] trust_level = "trusted"`) does **not** suppress the dialog - probed 2026-09-01, a launcher-shaped spawn still booted into "Hooks need review". The `cod` lane is back on the roster (codex-primary and codex-bianca, two panes each). If a codex pane ever shows the dialog again, the template has regressed: re-apply it, never answer in-pane.
2. **agy post-task feedback survey** ("How's the CLI experience so far?") - settled by config on 2026-09-02: `"showFeedbackSurvey": false` is set in both `~/.gemini/settings.json` and `~/.gemini/antigravity-cli/settings.json` (the field name is the one agy's own settings struct carries; which of the two files it reads was not proved, so both carry it). The `agy` lane is back on the roster (two panes). A pane showing the survey means the setting did not take: drop the lane, never answer in-pane.

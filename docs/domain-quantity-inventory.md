...
- `WindowSemanticKey`, `ModelId`, `GroupName`, `WindowScopeKind`, `WindowScope`, `ReportedResolution`, `QuantizationSemantics`, `WindowResetState`, `ResetGridId`, `WindowSeverity`, `NominalWindowDuration`, `ResetPrecision`, `MeterWindow`, `CreditHeadroomSelection`: window specification enums and composite structs. `ResetGridId` names the fixed grid a `WindowResetState::Scheduled` instant was computed from (`aub-ud17`), not a measured quantity. `GroupName` is a provider quota group's display name (`aub-n8yx`), the scope of one budget shared across a set of models the window identity cannot enumerate; it is deliberately not a `ModelId`, so a group window never matches `--model` selection. `ResetPrecision` is the reset-instant granularity an adapter declares for the provider surface it reads resets from (`aub-w1a0`), the same kind of declared-surface fact as `DocumentedGranularity` below; private representation, a public validated constructor, no `Default`, no `Display`.

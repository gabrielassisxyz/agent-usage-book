...
- `WindowSemanticKey`, `ModelId`, `GroupName`, `WindowScopeKind`, `WindowScope`, `ReportedResolution`, `QuantizationSemantics`, `WindowResetState`, `ResetGridId`, `WindowSeverity`, `NominalWindowDuration`, `MeterWindow`, `CreditHeadroomSelection`: window specification enums and composite structs. `ResetGridId` names the fixed grid a `WindowResetState::Scheduled` instant was computed from (`aub-ud17`), not a measured quantity. `GroupName` is a provider quota group's display name (`aub-n8yx`), the scope of one budget shared across a set of models the window identity cannot enumerate; it is deliberately not a `ModelId`, so a group window never matches `--model` selection.

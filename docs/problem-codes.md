...
stale-reason and report-qualification classifications, and every code maps to
exactly one exit class. The table is checked against the `ProblemCode` enum
by a test: renaming or removing a code, or adding one without a row here,
fails the build.

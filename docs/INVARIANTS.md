...
| 28 | At most one rate card is in force per vendor, model, token class, billing basis, window and instant: a money card and a percent-of-window card for one class coexist, and a money card has no window. | src/rate_book.rs | tests::two_scheduled_rows_overlapping_on_a_shared_day_are_refused |

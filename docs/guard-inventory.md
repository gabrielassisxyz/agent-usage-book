...
sh-17	aub-eun.2	shell	bin/checks/boundary-rules/13-no-async-runtime-dependency	python	Cargo.toml	NL = chr(10)\nt = open("Cargo.toml").read()\nold = "[dependencies]" + NL\nassert t.count(old) == 1, "dependencies header not unique"\nt = t.replace(old, old + 'tokio = "1.53.1"' + NL, 1)\nopen("Cargo.toml", "w").write(t)		Forbidden async runtime detected	bin/checks/70-quantity-inventory	0	every pub struct/enum
...
sh-33	aub-8av3	shell	bin/checks/boundary-rules/13-no-async-runtime-dependency	python	Cargo.toml	NL = chr(10)\nt = open("Cargo.toml").read()\nold = "[dependencies]" + NL\nassert t.count(old) == 1, "dependencies header not unique"\nt = t.replace(old, old + 'tokio = "1.99.0"' + NL, 1)\nopen("Cargo.toml", "w").write(t)		failed to select a version	bin/checks/70-quantity-inventory	0	every pub struct/enum
...
c-18	aub-gmms	cargo	cargo test --lib evidence::tests::quality_combine_never_recovers_measured_from_estimated	sed	src/evidence.rs	s/            (Self::Measured, Self::Measured) => Self::Measured,/            (Self::Measured, _) => Self::Measured,/		quality_combine_never_recovers_measured_from_estimated \.\.\. FAILED	cargo test --lib domain::tokens	0	test result: ok
c-19	aub-88b3	cargo	cargo test --lib meter::sampler::tests::bounded_concurrency_is_respected_and_reached	sed	src/meter/sampler.rs	s/        let worker_count = leased.len().min(self.max_concurrent_requests);/        let worker_count = leased.len();/		bound|concurrency|assertion|left.*right	cargo test --lib meter::sampler::tests::the_normalized_fingerprint_is_deterministic_and_distinguishes_windows	0	test result: ok
...
c-21	aub-yxsw	cargo	cargo test --test module_skeleton dependency_check_rejects_a_planted_forbidden_dependency	replace	tests/module_skeleton.rs	        && deps.contains_key(name)	        && deps.get(name).and_then(toml::Value::as_str).is_some()	declares_dependency\(.\[dependencies\.rusqlite\]	cargo test --test module_skeleton lowest_layers_reference_no_forbidden_crate	0	test result: ok
c-22	aub-2eda	cargo	git -c user.email=guard-mutations@example.invalid -c user.name=guard-mutations commit -qam "c-22: commit the mutation so git worktree add checks it out" && { cargo test --test build_script alternating_worktree_builds_with_a_shared_target_dir_never_fail; rc=$?; git reset -q HEAD~1; exit $rc; }	replace	build.rs	    let manifest_dir = std::env::var("CARGO_MANIFEST_DIR")\n        .expect("cargo must set CARGO_MANIFEST_DIR when running a build script");	    let manifest_dir = env!("CARGO_MANIFEST_DIR").to_string(); // probe: baked at compile time	build script must fail without a toolchain file	cargo test --test build_script toolchain_file_resolves_under_the_given_manifest_dir	0	test result: ok
c-23	aub-vcx.9	cargo	cargo test --lib build_info::tests::crate_version_matches_the_manifest_on_disk	replace	src/build_info.rs	pub fn crate_version() -> &'static str {	pub fn crate_version() -> &'static str {\n    return "0.0.0";	crate_version|assertion|left.*right	cargo test --lib build_info::tests::source_revision_is_a_full_lowercase_sha	0	test result: ok
c-24	aub-71j.2	cargo	cargo test --lib logging::tests::documented_event_vocabulary_matches_typed_enum	sed	docs/diagnostics.md	/^\| run_started |/d		has no row matching	cargo test --lib logging::tests -- --skip documented_event_vocabulary_matches_typed_enum	0	test result: ok
...
c-28	aub-eqpk	cargo	cargo test --lib tests::validation_skips_the_tracker_check_when_no_beads_directory_exists_at_all	replace	src/lib.rs	                Err(e) if e.kind() == std::io::ErrorKind::NotFound => {\n                    return Ok(InvariantRowValidation::TrackerMembershipSkipped);\n                }	                Err(e) if e.kind() == std::io::ErrorKind::NotFound => {\n                    return Err(format!("probe: tracker check disabled"));\n                }	TrackerMembershipSkipped|left == right	cargo test --lib tests::validation_fails_when_a_named_unenforced_bead_is_absent_from_a_present_tracker	0	test result: ok
c-29	aub-lh5r	cargo	cargo test --lib tests::every_invariant_names_existing_file_and_test_or_open_tracker_bead	sed	docs/INVARIANTS.md	/^| 1 |/ s|tests/backup.rs|tests/zz_no_such_file.rs|		zz_no_such_file|does not exist	cargo test --lib tests::invariants_document_matches_plan_section_42	0	test result: ok
c-30	aub-tjwn	cargo	bin/checks/30-test	python	src/lib.rs	import os\np = "src/lib.rs"\nt = open(p).read()\nold = "        if tracker_membership_skipped || tracker_absent {"\nassert t.count(old) == 1, "marker condition not found exactly once"\nnew = "        if true {" if os.path.exists(".beads/issues.jsonl") else "        if false {"\nopen(p, "w").write(t.replace(old, new, 1))		skip marker was (not )?surfaced	grep -qF 'INVARIANT_TRACKER_MEMBERSHIP_SKIPPED' bin/checks/30-test	0	
c-31	aub-eun.11	cargo	cargo test --test sampler_batch named_accounts_are_isolated_from_ambient_credentials	sed	src/meter/anthropic.rs	s/    let raw = credential.expose().trim();/    let raw = ""; \/\/ probe: explicit credential wiring removed/		left: AuthRequired|must carry explicit authorization|ambient credential must not reach	cargo test --test module_skeleton manifest_declares_no_forbidden_crate	0	test result: ok
...
c-38	aub-xgdl	cargo	cargo test --test schema_audit the_live_schema_meets_the_contract	sed	src/store/migrations/0012_usage_event_component.rs	s/    count INTEGER NOT NULL CHECK (count >= 0),/    count INTEGER NOT NULL,/		usage_component.count is a quantity column with no CHECK	cargo test --test schema_audit every_user_table_is_strict	0	test result: ok

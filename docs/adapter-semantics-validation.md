...
| Codex (`src/meter/codex.rs`) | the ChatGPT usage page for the configured account | whole percentage points, that is 10000 parts per million | `payload.rate_limits.primary` (account wide) and `payload.rate_limits.secondary` (account wide), read from the newest `rollout-*.jsonl` under the account's Codex home |

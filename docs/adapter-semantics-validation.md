...
| Anthropic status line (`src/meter/anthropic.rs`, contract `anthropic-statusline-rate-limits-v1`) | the Claude Code status line's own rendering of the same usage figures, teed into `<state.dir>/statusline/<account>.jsonl` by `aub statusline`: one JSON line per meter change carrying `received_at`, `session_id`, `cwd` and a `windows` map. The reader takes the file's last line, through the local-file transport. Freshness rule: a line counts when its `received_at` lies within the account's ordinary cadence (`sampling.default_interval`, 300 s by default) of the tick and never ahead of it; an older line is ignored and the usage endpoint runs. `anthropic.statusline = false` ignores the files and returns to the endpoint on every tick | whole percentage points (`used_percentage` converts to ppm by the same percent step the endpoint path uses), that is 10000 parts per million; `resets_at` is an integer epoch second, a `Known` reset | the line's `five_hour` (account wide) is mapped to `session` and `seven_day` (account wide) to `weekly_all`, so status-line and endpoint observations match by window identity; any further window name is mapped to nothing - no `meter_window` row is invented for it, and the name is recorded in the evidence capsule. A status-line observation is a subset source: an endpoint window missing from a status-line observation is not a window-set change (`aub-gnke`) |
| OpenCode Go (`src/meter/opencode.rs`) | a **scraped page**, not an API: the workspace page `https://opencode.ai/workspace/<id>/go` as seen signed in in the browser. The provider has no usage endpoint (the reference, `https://ai.rud.is/posts/2026-06-06-opencode-go-usage/`, states none exists), so the meters are read from the page's rendered markup, matching the reference tool's own parser (`git.sr.ht/~hrbrmstr/opencode-go-usage`, `usage/usage.go`) rather than the post's JSON description: one `<div data-slot="usage-item">` per window, the parser keys on the literal marker string `data-slot="usage-item"`; a page without any such element is schema drift. The operator reads the same page in the browser for the comparison | one-tenth-of-a-percent decimal points, that is 1000 parts per million; the percent comes from each item's `aria-valuenow` attribute at one decimal place. The reset instant the adapter derives is precise to one hour (3600 s), because the page's rendered reset text (`Resets in 6 days 8 hours`) floors the remaining time to whole hours: the adapter declares that precision (`OpenCodeAdapter::RESET_PRECISION_SECONDS`) and the window-anomaly classifier holds two consecutive derived resets within it plus the gap between the observations to be one unchanged boundary (`aub-w1a0`) | `rolling` (account wide), `weekly` (account wide), `monthly` (account wide) |

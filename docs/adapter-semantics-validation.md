...
| OpenCode Go (`src/meter/opencode.rs`) | a **scraped page**, not an API: the workspace page `https://opencode.ai/workspace/<id>/go` as seen signed in in the browser. The provider has no usage endpoint (the reference, `https://ai.rud.is/posts/2026-06-06-opencode-go-usage/`, states none exists), so the meters are read from the page's embedded initial state script, which the parser keys on by the literal marker string `reset_in_sec`; a page without that marker is schema drift. The operator reads the same page in the browser for the comparison | integer percentage points, that is 10000 parts per million; the percent is a whole number by the page contract | `rolling` (account wide), `weekly` (account wide), `monthly` (account wide) |

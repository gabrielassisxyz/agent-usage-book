...
impl Command {
    ...
    pub fn flag_policy(self) -> FlagPolicy {
        match self {
            ...
            Command::Calibrate => FlagPolicy {
                format: FlagSupport::Rejected {
                    reason: "calibrate prints plain operational lines, not a report",
                },
                explain: FlagSupport::Rejected {
                    reason: "calibrate records an experiment premise, not a derived quantity",
                },
                account: FlagSupport::Accepted,
                model: FlagSupport::Rejected {
                    reason: "a calibration experiment is scoped by window, not by model",
                },
                no_color: FlagSupport::Rejected {
                    reason: "calibrate prints no color",
                },
                verbosity: FlagSupport::Accepted,
            },
        }
    ...
    pub fn summary(self) -> Option<&'static str> {
        match self {
            ...
            Command::Calibrate => {
                Some("record a controlled calibration experiment without a resident process")
            }
        }
    }
    ...
    pub fn question(self) -> Option<&'static str> {
        match self {
            ...
            Command::Calibrate => {
                Some("what is the state of the controlled calibration experiment?")
            }
            Command::LoggingFixture | Command::StateCheck | Command::ExitClass => None,
            Command::AttemptCrashHook => None,
            Command::ProjectionCrashHook => None,
    ...
    pub fn options_help(self) -> Option<&'static str> {
        match self {
            ...
            Command::Calibrate => Some(
                "begin --account NAME --plan-tier TIER --window KEY --cost-model ID [--expect-kinds K,...] [--experiment ID] --assert-exclusive | status [--experiment ID] | end [--experiment ID]",
            ),
            Command::Status
...
fn calibrate_command(clock: &impl Clock, invocation: &Invocation) -> Result<(), Error> {
    ...
    match subcommand {
        ...
        other => Err(Error::Usage(format!(
            "calibrate requires a subcommand (begin | status | end), got {other:?}"
        ))),
    }
}

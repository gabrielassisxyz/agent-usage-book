...
| `docs/forbidden-patterns.txt` | the single source the four identity scans read (`tests/fixture_corpus_audit.rs`, `src/transcripts/native.rs`, `bin/checks/82-identity-privacy-scan`, `tests/identity_privacy_scan.rs`), so a bead that needs the list at runtime must never ship it contiguous in the binary: the scan reads the release binary with `strings` and treats any pattern it finds there as a leak. Reuse one of the two devices already in the tree, the reversed key prefix in `src/meter/evidence.rs` or the compile-time reversed list in `src/domain/failure.rs`, and run `bin/checks/82-identity-privacy-scan` after your last commit, since it archives `HEAD` |

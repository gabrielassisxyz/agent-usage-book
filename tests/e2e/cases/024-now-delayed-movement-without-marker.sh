...
CASE_DESCRIPTION="A fresh heartbeat with no explicit marker never becomes active-session evidence in aub status, whatever the meter is doing."
...
case_steps() {
    ...
    step "status-heartbeat-without-marker" env \
        "HOME=$STATE_DIR/home" \
        "AUB_STATE_DIR=$STATE_DIR" \
        "AUB_CONFIG_FILE=$STATE_DIR/aub.toml" \
        "$AUB_BIN" status --account work-primary --session-id "claude-code:sess-delayed-1" --format json

    # 4. A second named status read still changes nothing about the activity
    #    claim: it is composed only from markers and heartbeats.
    step "status-after-more-movement" env \
        "HOME=$STATE_DIR/home" \
        "AUB_STATE_DIR=$STATE_DIR" \
        "AUB_CONFIG_FILE=$STATE_DIR/aub.toml" \
        "$AUB_BIN" status --account work-primary --session-id "claude-code:sess-delayed-1" --format json
